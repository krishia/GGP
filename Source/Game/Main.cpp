...
INT WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ INT nCmdShow)
{
    ...
    std::shared_ptr<library::Model> suit = std::make_shared<library::Model>(L"nanosuit/nanosuit.obj");
    if (FAILED(game->GetRenderer()->AddRenderable(L"suit", suit)))
    {
        return 0;
    }
    if (FAILED(game->GetRenderer()->SetVertexShaderOfRenderable(L"suit", L"PhongShader")))
    {
        return 0;
    }
    if (FAILED(game->GetRenderer()->SetPixelShaderOfRenderable(L"suit", L"PhongShader")))
    {
        return 0;
    }
